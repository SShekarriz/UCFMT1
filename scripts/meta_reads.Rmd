...
org <- read.csv(paste(proj, "data/Metaphlan4/merged_metaphlan4.txt", 
                      sep = ""),
                sep = "\t", header = F, 
                comment.char = "#")
org %>%
...
ggsave(paste(proj, "figs/CESp.png", sep = ""),
       width = 10, height = 8, units = "cm")
...
metaphlan %>%
  ...
  mutate(abundance= as.numeric(abundance))-> tbl
...
# get ID order, sorted by each ID's abundance in the most abundant class
  ...
  arrange(desc(abundance)) %>% distinct(Study_ID) %>%
  pull(Study_ID) -> ID.order
...
ggplot(aes(abundance, Fig_lab, fill=Family_lab)) +
...
ggplot(aes(abundance, sample, fill=Family_lab)) +
  geom_bar(stat = "identity") +
