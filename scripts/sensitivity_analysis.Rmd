...
This script is for running the sensitivity analysis on genes, and possibly other
features as well, to determine if the cutoffs we chose materially effect our
results.

Before running the analysis, I will look at the gene results and decide what I
think it means for them to be "materially affected".

Our gene results:

- The number of apparently engrafted genes in FMT is greater than apparently
  engrafted genes in Placebo, p <= 0.001. This p-value should remain low (say <=
  0.01), and the sign of the effect should be unchanged
- The current number of genes engrafted in 3 or more individuals is 45,419. This
  number should be stable within an order of magnitude (I don't feel super
  strongly about this. I think it's okay if it changes quite a bit, and it's the
  thing that is going to be the most sensitive to the cutoffs)
- The following proportions should be pretty stable (certainly within an order
  of magnitude, ideally within a factor of 2):
  - Proportion present in both FMT and Placebo: 56.9%
  - Proportion exclusive to FMT: 37.9%
  - Proportion exclusive to Placebo: 5.2%
  - Proportion unique to FMT responders: 3.3%
  - Proportion unique to FMT non-responders: 0.6%
  - It's the ratios that matter here. The number unique to placebo is much
	smaller than the number unique to FMT, and the number unique to responders
	is much smaller than the number unique to non-responders.
...
THIS IS WHERE I STOPPED. I STILL NEED TO:

- RUN PERMUTATION TESTS ON ALL MATRICES
- GET THE OBSERVED TEST STATISTIC ON ALL MATRICES
- DRAW GRAPHS FOR ALL MATRICES
...
# tst = get_profile_pat(long_mark_ge, mapfile_mgm, all_cutoff$abs[2],
#                       all_cutoff$pres[2], Treatment %in% c('FMT', 'Placebo'))

for (i in 1:nrow(all_cutoff)){
    print(i)
    arr_prof[i,,] = get_profile_pat(long_mark_ge, mapfile_mgm, 
